...
class BookService
{
   JDOBookStorage bookStorage 
   
   @Path('{id}')
   @GET
   @Produces('application/json')
   Book get(@PathParam('id') Long id) throws BookNotFoundException
   {
...
//   }
   ...
   @POST
   @Consumes('application/json')
   public Response post( Book book, @Context UriInfo uriInfo)
   {
	  String id = bookStorage.postBook(book)
	  URI location = uriInfo.getBaseUriBuilder().path(getClass()).path(id).build()
	  Response.created(location).entity(location.toString()).type('text/plain').build()
   }
}